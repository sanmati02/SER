...
def build_optimizer(params, configs):
    ...
    logger.info(f'Successfully created optimizer: {use_optimizer} with arguments: {optimizer_args}')
    return optimizer
...
def build_lr_scheduler(optimizer, step_per_epoch, configs):
    ...
    # Automatically set additional parameters if missing
    ...
    logger.info(f'Successfully created learning rate scheduler: {use_scheduler} with arguments: {scheduler_args}')
    return scheduler
